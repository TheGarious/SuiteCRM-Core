...
class ViewedEmailTrackerParser implements CampaignEmailParserInterface
{
    ...
    {
        return 'viewed-email-tracker-parser';
    }
    ...
    public function parse(Record $record, array $context): void
    {
        ...
        if ($trackerId === null) {
            $this->logger->debug(
                'Campaigns:ViewedEmailTrackerParser::parse - missing trackerId - id: ' . $trackerId, [
                    'trackerId' => $trackerId,
                ]
            );
            return;
        ...
        $attributes['description'] = $this->trackerManager->addViewedTracker($trackerId, $attributes['description'] ?? '');
        $attributes['description_html'] = $this->trackerManager->addViewedTracker($trackerId, $attributes['description_html'] ?? '');
        $record->setAttributes($attributes);

        $this->logger->debug(
            'Campaigns:ViewedEmailTrackerParser::parse - trackerId: ' . $trackerId, [
                'trackerId' => $trackerId,
